...
class WSLSAgent():
    ...
    def __init__(self, n_envs, wait_time_for_reward, odor_cues_indices, patch_cue_idx):
        ...
        self.n_envs = n_envs
        self.stay_in_current_patch = np.zeros((n_envs)).astype(bool)


    def sample_action(self, observations):
        if self.last_observations is not None:
            patch_cue_diff = observations[:, self.patch_cue_idx] - self.last_observations[:, self.patch_cue_idx]
            self.stay_in_current_patch = np.where(patch_cue_diff > 0, True, self.stay_in_current_patch)
    ...
    def reset_state(self):
        ...
        self.stay_in_current_patch = np.zeros((self.n_envs)).astype(bool)